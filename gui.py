...
def pressSendRequestBtn():
    ...
    response = httpClient.sendRequest(urlContent, option.get())
...
def createWindow(title, size):
    ...
    createFormFrame(window)
    createHeaderFrame(window)
    createBodyFrame(window)
    return window


#Creamos el frame donde estara el form para introducir los datos
def createFormFrame(window):    
